...
var rayColors = new THREE.Color("rgb(100%, 0%, 0%)");
...
function generateLighting() {
	...
	shadowLight.target.position.set(10, 0, 10);
	//shadowLight.lookAt(-10, 10, -10);
	// Variables used to create the back light
	var backLightColor = colors.WHITE;
	var backLightIntensity = 0.25;
...
function generateBuildingBlock(
	...
	if (isTall(geometryParameters.height) || numOfDivisions < 1) {
		...
		if (
			...
			buildingArray.push(building.getMergedMesh());		
...
function getRaysPark() {
	...
	var hit =  0;
	if(!playAnim) {
		console.log(parkArray.length);
		for(var i=0; i < parkArray.length;) {
			var x = parkArray[i].position.x;
			var y = parkArray[i].position.y;
			var z = parkArray[i].position.z;
			var origin = new THREE.Vector3(x,y,z);
			var target = shadowLight.position.clone();
			var direction = new THREE.Vector3().subVectors( target, origin ).normalize();
			
			raycaster.set(origin, direction );
			
			// calculate objects intersecting the picking ray
			var intersects = raycaster.intersectObjects( buildingArray);
			console.log(intersects.length);
			if(intersects.length > 0) {
				for ( var j = 0; j < intersects.length; j++ ) {
					var boxHelper = new THREE.BoxHelper(intersects[j].object, rayColors);
					scene.add( boxHelper );
					var boxHelper1 = new THREE.BoxHelper(parkArray[i], rayColors);
		...
		lable.innerHTML = "Park Visibility";
		value.innerHTML = hit/parkArray.length;
		console.log(hit/parkArray.length);
		renderer.render( scene, camera );
	} else {
		alert("Pause Animation First");
	}
	...
	// sky Visibility
function getSkyVisibility() {
		var hit =  0;
		var skyWidth = 0;
		for(var h=0; h < park.length; h++) {
			var x_coords = skyPlane.geometry.parameters.width;
			var y_coords = skyPlane.geometry.parameters.height;
			for(var i=skyPlane.position.x - (x_coords-1)/2; i < skyPlane.position.x + x_coords/2;) {
				for(var j=skyPlane.position.z - (y_coords/2); j < skyPlane.position.z + y_coords/2;) {
					var target = new THREE.Vector3(i,skyPlane.position.y,j);
					var origin = park[h].position.clone();
					var direction = new THREE.Vector3().subVectors( target, origin ).normalize();
					...
					if(intersects.length > 0) {
						for ( var k = 0; k < intersects.length; k++ ) {
							var boxHelper = new THREE.BoxHelper(intersects[k].object, rayColors);
							scene.add( boxHelper );
							var arrowHelper = new THREE.ArrowHelper( direction, origin, intersects[k].distance, rayColors );
		...
		lable.innerHTML = "Sky Visibility";
		value.innerHTML = hit/skyWidth;
		renderer.render( scene, camera );
		
	}


//raytracing to landmark
function renderRayForLandmark() {
	 // update ray with the landmark and mouse position
	 var hit = 0;
	 for(var d=0; d <= 360; d ++){
	   for (var h= 100; h < landMark.position.y;) {
		 var angleRad = (d * Math.PI) / 180;
		 var x_axis = Math.sin(angleRad);
		 var y_axis = Math.cos(angleRad);
		 var origin = landMark.position.clone();
		 origin.y = h;
		 var direction = new THREE.Vector3(x_axis,0,y_axis).normalize();
		 raycaster.set(origin, direction );
		 // calculate objects intersecting the picking ray
		 var intersects = raycaster.intersectObjects( buildingArray, true );
		 if ( intersects.length > 0 ) {
		 for ( var i = 0; i < intersects.length; i++ ) {
				var boxHelper = new THREE.BoxHelper(intersects[i].object, rayColors);
				scene.add( boxHelper );
			  	hit++; 
		 }
	   }
	 ...
	 lable.innerHTML = "Landmark Visibility";
	 value.innerHTML = hit/buildingArray.length;
	 console.log(hit/buildingArray.length);
	 renderer.render( scene, camera );
	}

	pauseButton.onclick = function pauseAnimation() {
	if(!runAnim) {
...
function generateSkyPlane() {
	...
	geometry = new THREE.BoxGeometry(
		baseGeometryParams.width,
		baseGeometryParams.height,
		baseGeometryParams.depth
	);

	// Generate the mesh and return it

	skyPlane = new THREE.Mesh(geometry, material);
	skyPlane.position.set(basePosition.x, basePosition.y, basePosition.z);
	scene.add(skyPlane);
...
document.getElementById("shadowMap").addEventListener(
	"click",
	function() {getRaysPark()}
);

document.getElementById("skyVisibility").addEventListener(
	"click",
	function() {getSkyVisibility()}
);
...
var render = function () {
	if(playAnim) {
		...
		backLight.position.x = 500 * Math.sin(Date.now() / 5000);
		backLight.position.z = 500 * Math.cos(Date.now() / 5000);
		shadowLight.position.y = 500 * Math.sin(Date.now() / 500);
		shadowLight.position.x = 500 * Math.cos(Date.now() / 500);
	}
